...
function scrollHandler(event) {
    event.target.querySelectorAll('.slide-wrapper').forEach((node, number) => {
        ...
        let dim = 0;
        if (document.scrollingElement.scrollTop < dimmThreshold) dim = 100;
        else if (document.scrollingElement.scrollTop > sectionOffset) dim = 0;
        else {
            ...
            dim = ((sectionOffset - document.scrollingElement.scrollTop) / (window.innerHeight / 100));
        ...
        // if (number === 2) console.log(document.scrollingElement.scrollTop, sectionOffset, dimmThreshold, dim);
        if (node.dim !== dim) {
